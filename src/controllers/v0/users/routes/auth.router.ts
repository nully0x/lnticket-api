...
const { createHash, associateTokenToHash } = require('../../../../utils/lnauthservice');
...
router.get('/get-login-url', async (req: Request, res: Response): Promise<unknown> => {
    try {
        const data = await LnurlAuthService.generateAuthUrl();

        const session_token:Buffer = await new jose.SignJWT({ hash: data.secretHash })
